...
pthread_mutex_t lockInitializer = PTHREAD_MUTEX_INITIALIZER;
...
void *PingerPonger(void *tidptr)
{
        ...
        for (unsigned dex = 0; dex <= NumRounds; dex++) {
                pthread_mutex_lock(&lockInitializer);
                while(tid == PrevVal)
                {
                        pthread_cond_wait(&condInitializer, &lockInitializer);
                }
                pingpongprint(tid);
                pthread_cond_signal(&condInitializer);
                pthread_mutex_unlock(&lockInitializer);
        }
        return NULL;
}
